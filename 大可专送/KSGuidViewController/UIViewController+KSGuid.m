...
- (UIButton*)removeBtn{
    ...
    CGFloat btnY = CGRectGetMaxY(self.view.frame) * 0.78;
    removeBtn.frame = CGRectMake(btnX, btnY, btnW, btnH);
    removeBtn.layer.cornerRadius = 0;
...
//    removeBtn.titleLabel.font = [UIFont systemFontOfSize:18.];
    [removeBtn setImage:[UIImage imageNamed:@"启动页_btn"] forState:0];
     //********************自定义结束**********************************//
...
+ (void)load{
    ...
    NSString* versionCache =@"FIRSTIN_KEY";// [[NSUserDefaults standardUserDefaults] objectForKey:FIRST_IN_KEY];
    //启动时候首先判断是不是第一次
