...
@interface sendViewController ()<MapToolDelegate,CNContactPickerDelegate,UIActionSheetDelegate>
...
-(void)collectBtnClick:(UIButton *)btn{
    mapAddressModel * model= btn==_collectBtn1? _sendModel:_receiptModel;
    if (model==nil) {
        return;
    }
...
-(void)checkAddress{
    if (_receiptModel.location.latitude && _sendModel.location.latitude) {
     
        [[mapViewTool shared] getNaviWithStartLocation:_receiptModel.location withEndLocation:_sendModel.location];
    }else{
        self.freeModel=nil;
    }
}
...
-(void)changeAddressBtnClick{
    mapAddressModel * model= self.receiptModel;
    self.receiptModel=_sendModel;
...
- (void)viewDidLoad {
    ...
    [_timeAndGoodTypeView makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(_goodInfoView.mas_bottom).offset(10);
        make.left.right.mas_equalTo(self.view);
        make.height.mas_equalTo(90);
...
- (void)commitOrder{
    ...
    if (_sendModel.cityid==nil || _receiptModel.cityid==nil) {
        [SVProgressHUD showErrorWithStatus:@"所选城市未开放"];
        return;
    }
    if (![_sendModel.cityid isEqualToString: _receiptModel.cityid]) {
    ...
    NSDictionary * dict=@{@"ordertype":@"2",
                           ...
                           @"source":@"3",
                           @"remark":mark,
                           @"address1":_sendAddressTextField.text,
...
-(UIView *)addressView{
    if (!_addressView) {
        ...
        _sendAddressTextField.placeholder=@"请选择发货地址";
...
-(UIView *)phoneView{
    if (!_phoneView) {
        ...
        _phoneTextField1.placeholder=@"请输入发货人电话";
        _phoneTextField1.text=APP_NAME;
